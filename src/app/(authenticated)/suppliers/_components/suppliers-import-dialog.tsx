...
import { importSuppliers } from "../_actions/supplier";
  ...
  const onSubmit = async () => {
    ...
    if (file && file[0]) {
      ...
      try {
        ...
        const suppliers = await extractExcelData(file[0]);

        // Importar clientes
        if (authUser) {
          const result = await importSuppliers(authUser.id, suppliers);
    ...
    >
      <DialogContent className="gap-2 sm:max-w-sm">
        <DialogHeader className="text-start">
          ...
          <DialogDescription>
            Importar proveedores rápidamente desde un archivo de Excel.
          </DialogDescription>
        </DialogHeader>
              ...
              <CollapsibleContent className="space-y-2">
                <div className="rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-800 dark:bg-red-950 mt-2">
                  <ul className="text-xs text-red-700 dark:text-red-300 space-y-1 list-disc list-inside">
                    <li>
                      Una vez importados los proveedores no hay vuelta atrás
                    </li>
                    <li>
                      Los proveedores sin compras registradas en la app serán
                      eliminados al momento de la importación
                    </li>
                    <li>
                      Si un proveedor existe en la app y en el Excel (mismo
                      número de nit o nombre), solo se actualizarán los datos
                    </li>
                    <li>
                      Los proveedores nuevos del Excel se añadirán
                      automáticamente
                    </li>
                  </ul>
                </div>
        ...
        <DialogFooter className="gap-2">
          ...
          <Button disabled={isLoading} type="submit" form="client-import-form">
            ...
            {isLoading ? "Importando proveedores..." : "Importar proveedores"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>

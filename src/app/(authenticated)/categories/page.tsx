...
import { CategoryDialogs } from "./_components/category-dialogs";
...
export async function generateMetadata(): Promise<Metadata> {
  return {
    title: "Leviatan POS | Gestión de categorías",
  };
}
...
export default async function Categories() {
  ...
  if (!permisosUser.includes("CATEGORIAS")) {
  ...
  const { categorias } = await getCategories(data.user.id);
        ...
        <div className="flex flex-wrap items-end justify-between gap-2">
          <div>
            ...
            <p className="text-muted-foreground">
              Gestiona todas las categorías de los productos aquí.
            </p>
          </div>
          <CategoryPrimaryButtons />
        </div>
        <CategoryTable data={categorias} />
      </Main>
